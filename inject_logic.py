...
html_path = 'KmlArcGenerator_Standalone.html'
with open(html_path, 'r', encoding='utf-8') as f:
    content = f.read()

# CSS 업데이트 (스크롤바, 모던 다크 테마 추가 조율)
css_addition = """
.marker-list::-webkit-scrollbar { width: 8px; }
.marker-list::-webkit-scrollbar-track { background: rgba(255,255,255,0.05); border-radius: 4px; }
.marker-list::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius: 4px; }
.sector-item { background: rgba(255,255,255,0.03); border: 1px solid var(--border-color); border-radius: 0.8rem; padding: 1rem; margin-bottom: 1rem; }
.sector-header { display: flex; justify-content: space-between; align-items: center; cursor: pointer; }
.sector-header h3 { margin: 0; font-size: 1rem; color: var(--text-primary); display: flex; align-items: center; gap: 0.5rem; }
.sector-body { margin-top: 1rem; display: none; }
.sector-body.open { display: block; }
.remove-sector-btn { color: #ef4444; background: none; border: none; cursor: pointer; font-size: 1.2rem; }
.remove-sector-btn:hover { color: #b91c1c; }
.sector-color-badge { width: 14px; height: 14px; border-radius: 50%; display: inline-block; }
.tooltip-label { background: transparent; border: none; box-shadow: none; color: white; font-weight: bold; text-shadow: 1px 1px 2px black, -1px -1px 2px black, 1px -1px 2px black, -1px 1px 2px black; font-size: 12px; }
"""

# HTML UI 및 Script 교체
replacement = """<div class="form-section">
                <form id="kmlForm">
                    <div class="card">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                            <h2 style="margin-bottom: 0;">원호(부채꼴) 목록</h2>
                            <button type="button" id="addSectorBtn" class="add-btn" style="padding: 0.4rem 0.8rem; font-size: 0.9rem;">+ 추가</button>
                        </div>
                        <div id="sectorList" class="marker-list" style="max-height: 500px; padding-right: 0.5rem; overflow-y: auto;">
                            <!-- 동적 부채꼴 추가 -->
                        </div>
                        <div class="settings-group" style="margin-top: 1rem; border-top: 1px solid var(--border-color); padding-top: 1rem;">
                            <div class="input-box">
                    ...
                    <!-- 커스텀 마커 영역 -->
                    <div class="card">
                        <div class="card-header-row" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.8rem;">
                            <h2 style="margin-bottom: 0;">커스텀 마커 (목표점)</h2>
                            <div style="display: flex; gap: 1rem; align-items: center;">
                                <label class="toggle-switch" title="마커 숨기기/보이기">
                                    <input type="checkbox" id="markerVisibilityToggle" checked>
                                    <span class="slider"></span>
                                    <span class="toggle-label">보이기</span>
                                </label>
                                <label class="toggle-switch" title="클릭하여 지도에 마커 찍기 모드 활성화">
                                    <input type="checkbox" id="markerModeToggle">
                                    <span class="slider"></span>
                                    <span class="toggle-label">지도에서 찍기</span>
                                </label>
                            </div>
                        </div>
                        <div class="marker-add-row" style="flex-direction: column;">
                            <div class="coord-group" style="margin-bottom: 0;">
                                <div class="input-row">
                                    <label style="flex: 0 0 40px; font-size: 0.8rem; margin:0;">위도</label>
                                    <div class="dms-inputs">
                                        <input type="number" id="mLatD" value="37" min="0" max="90"><span>°</span>
                                        <input type="number" id="mLatM" value="33" min="0" max="59"><span>'</span>
                                        <input type="number" id="mLatS" value="59" min="0" max="59.99" step="0.01"><span>"</span>
                                        <select id="mLatDir">
                                            <option value="N">북위 (N)</option>
                                            <option value="S">남위 (S)</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="input-row" style="margin-top: 0.5rem;">
                                    <label style="flex: 0 0 40px; font-size: 0.8rem; margin:0;">경도</label>
                                    <div class="dms-inputs">
                                        <input type="number" id="mLonD" value="126" min="0" max="180"><span>°</span>
                                        <input type="number" id="mLonM" value="58" min="0" max="59"><span>'</span>
                                        <input type="number" id="mLonS" value="40" min="0" max="59.99" step="0.01"><span>"</span>
                                        <select id="mLonDir">
                                            <option value="E">동경 (E)</option>
                                            <option value="W">서경 (W)</option>
                                        </select>
                                    </div>
                                </div>
                                <button type="button" id="addMarkerBtn" class="add-btn" style="margin-top: 0.8rem; width: 100%; padding: 0.6rem;">마커 추가</button>
                            </div>
                        </div>
                        <div id="markerList" class="marker-list" style="margin-top: 1rem;">
                            <!-- 마커 동적 추가 영역 -->
                        </div>
                    </div>

                    <div class="button-group">
                        <button type="button" id="viewBtn" class="generate-btn view-btn">KML 코드 보기</button>
                        <button type="submit" id="downloadBtn" class="generate-btn">KML 파일 다운로드</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- KML 소스보기 모달창 -->
    <div id="kmlModal" class="modal">
        <div class="modal-content">
            <span class="close-btn">&times;</span>
            <h3>📄 KML 소스 코드</h3>
            <textarea id="kmlOutput" readonly></textarea>
            <button type="button" id="copyBtn" class="generate-btn">클립보드에 복사</button>
        </div>
    </div>

    <!-- CORS 방지용 오프라인 세계 지도 (GeoJSON) 데이터 -->
    <script>
const countriesData = undefined;
</script>
    <script>
const statesData = undefined;
</script>
    <script>
const citiesData = undefined;
</script>
    <script>
// 전역 변수
let map;
...
function addCustomMarker(lat, lon) {
    ...
    state.customMarkers.push({ id, lat, lon });
    ...
    state.customMarkers.forEach((m, index) => {
        ...
        item.innerHTML = `
            <div style="display:flex; width:100%; justify-content:space-between; align-items:center;">
                ...
                <button type="button" class="remove-btn" data-id="${m.id}" title="마커 삭제">&times;</button>
            </div>
            <div class="dms-inputs" style="width:100%; margin-top:0.3rem;">
                ...
                <input type="number" class="marker-dms-edit m-lat-s" value="${latDms.s}" min="0" max="59.99" step="0.01" data-id="${m.id}"><span>"</span>
            ...
            <div class="dms-inputs" style="width:100%; margin-top:0.3rem;">
                ...
                <input type="number" class="marker-dms-edit m-lon-s" value="${lonDms.s}" min="0" max="59.99" step="0.01" data-id="${m.id}"><span>"</span>
...
# 파일 교체 (form-section 위치 찾기)
start_idx = content.find('<div class="form-section">')
head_idx = content.find('</style>')

if start_idx != -1 and head_idx != -1:
    # head 쪽에 CSS 삽입
    content = content[:head_idx] + css_addition + content[head_idx:]
    
    # 다시 start_idx를 찾습니다 (content 길이가 변경되었으므로)
    start_idx = content.find('<div class="form-section">')
    ...
    print("Error: Could not find target tags.")
